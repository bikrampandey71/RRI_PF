...
def DrawHydro(PresentTimeTxt, ConvQ2H, HomeDir, Hydro_f_type, Hydro_f, Hydro_locNo, RsltDir, ObsData, \
    ...
    fp = font_manager.FontProperties(fname=r'C:\Users\bikra\times new roman.ttf')
    ...
    if AllParticleFlg == 1:
        ...
        for iPn in range(Pn):
            ...
            ax1.plot(ls_AllHydroDatetime, ls_iPnHydroData,lw=1.0, alpha=0.5, linestyle="solid", color='orange',
             label='BC_GFS All Particle W.L' if iPn == 0 else "")
    ...
    if ForecastHyd_BestP_Flg != 0:
        ...
        ax1.plot(ls_Datetime_HydBestP, ls_Hydro4Fig,lw=2.5,alpha=1.0,label='BC_GFS Best Particle W.L',linestyle="dotted",markersize=3,color='blue')
    #########Forecasted wl/qr for BestP ##################################
    ...
    if ForecastHyd_OrgP_Flg != 0:
        ...
        if ForecastHyd_OrgP_Flg == 2:
            ...
            NowErrVal = NowObsVal - NowCalcVal
            ls_Hydro4Fig = ls_Hydro4Fig + NowErrVal
        ax1.plot(ls_Datetime_HydOrgP, ls_Hydro4Fig,lw=1.5,alpha=1.0,label='BC_GFS Sim W.L',linestyle="dashed",markersize=3,color='Red')
    #########Forecasted wl/qr for OrgP ##################################
    ...
    if ForecastHyd_WtMean_Flg != 0:
        ...
        if ForecastHyd_WtMean_Flg == 2:
            ...
            NowErrVal = NowObsVal - NowCalcVal
            ls_Hydro4Fig = ls_Hydro4Fig + NowErrVal
        ax1.plot(ls_Datetime_HydWtMean, ls_Hydro4Fig,lw=1.5,alpha=1.0,label='BC_GFS Mean Particle W.L',linestyle="solid",markersize=5,color='Red')
    #########Forecasted wl/qr for weight mean ##################################
    ...
    ax1.legend(prop=fp, facecolor='white', framealpha=1, edgecolor='black', loc='upper right', bbox_to_anchor=(1, 0.85))
    #ax2.legend()
...
# ==============================
# Loop over a specific start/end (adds to your script)
# ==============================
...
def run_batch_fixed_range(HomeDir, RsltDir, ObsData,
    ...
    for ts in daterange_ts(start_dt, end_dt, pf_dt_min):
        idx += 1
        try:
            pl = DrawHydro(PresentTimeTxt=ts,
                           ConvQ2H=None,
                           HomeDir=HomeDir,
...
# ==============================
# Main entry (edit paths & times here)
# ==============================
if __name__ == "__main__":
    # ==== EDIT THESE PATHS / SETTINGS ====
    HOME_DIR   = r"C:\Users\bikra\Desktop\DA_PF_RRI\with_DA\Daily\Metrics\2024\N1_wl_2024"           # has PythonCode/DrawingConfig.ini
    RSLT_DIR = os.path.join(HOME_DIR, "Results")            # root with YYYY/MM/DD subfolders
    OBS_FILE = os.path.join(HOME_DIR, "ObsData/WaterLevel/ObsWL.csv")   # observed WL CSV   

    START_TS   = "202406200000"  # inclusive; format YYYYMMDDHHMM
    END_TS     = "202409300000"  # inclusive
    ...
    OUT_DIR    = os.path.join(HOME_DIR, "Results/Drawing_noDA")
    # =====================================
